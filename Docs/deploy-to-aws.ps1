...
# 3. Database initialized (run Db_Schema.sql)
# 4. Update variables below with your values
#
...
$DB_USERNAME = "admin"
$DB_NAME = "Condominio2"
$JWT_SECRET = "clave_super_secreta_para_jwt_1234567890_PRODUCTION"  # CHANGE THIS!
...
Set-Location "C:\Jose\code\FullStackMaster\MSD_CondoAdminBE\CondominioAPI"
...
try {
    ...
    $DB_SG_ID = (aws ec2 describe-security-groups `
        --filters "Name=group-name,Values=condominio-db-sg" `
...
try {
    ...
    $ECS_SG_ID = (aws ec2 describe-security-groups `
        --filters "Name=group-name,Values=condominio-ecs-sg" `
        --query "SecurityGroups[0].GroupId" `
        --output text `
        --region $AWS_REGION)
...
try {
    ...
    $ALB_SG_ID = (aws ec2 describe-security-groups `
        --filters "Name=group-name,Values=condominio-alb-sg" `
        --query "SecurityGroups[0].GroupId" `
        --output text `
        --region $AWS_REGION)
...
aws ec2 authorize-security-group-ingress `
    ...
    --source-group $ECS_SG_ID --region $AWS_REGION 2>$null

aws ec2 authorize-security-group-ingress `
    --group-id $ECS_SG_ID --protocol tcp --port 8080 `
    --source-group $ALB_SG_ID --region $AWS_REGION 2>$null
...
"@
    ...
    $SECRETS_POLICY | Out-File -FilePath secrets-policy.json -Encoding utf8
    
    aws iam put-role-policy `
        --role-name condominioECSExecutionRole `
        --policy-name SecretsManagerAccess `
        --policy-document file://secrets-policy.json 2>$null
